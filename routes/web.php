...
Route::middleware(['admin.auth'])->prefix('admin')->group(function () {
    ...
    // Tools & Calculators
    // ─────────────────────────────────────────────────────────────
    Route::prefix('tools')->name('tools.')->group(
        function () {
    ...
    // ─────────────────────────────────────────────────────────────
    Route::prefix('chat')->middleware('admin.permission:chat.access')->group(
        function () {
    ...
    Route::post('leads/{lead}/message', [LeadController::class, 'sendMessage'])
    ...
    Route::prefix('settings/meta')->name('settings.meta.')->middleware(['admin.permission:meta_leads.settings', 'throttle:meta-api'])->group(
        function () {
            ...
            Route::post('/{account}/toggle', [MetaSettingsController::class, 'toggle'])
                ->name('toggle');

            Route::post('/{account}/refresh', [MetaSettingsController::class, 'refresh'])
                ->name('refresh');

            Route::delete('/{account}', [MetaSettingsController::class, 'destroy'])
                ->name('destroy');

            Route::post('/test-webhook', [MetaSettingsController::class, 'testWebhook'])
                ->name('test-webhook');
        }
    );
    ...
    // ─────────────────────────────────────────────────────────────
    Route::prefix('settings/security')->name('settings.security.')->middleware(['admin.permission:settings.manage'])->group(
        function () {
    ...
    // ─────────────────────────────────────────────────────────────
    Route::prefix('gold-tracking')->name('gold-tracking.')->middleware('admin.permission:gold_tracking.view')->group(
        function () {
            // Dashboard
    ...
    Route::prefix('jewellery-stock')->name('jewellery-stock.')->group(function () {
        Route::get('/', [JewelleryStockController::class, 'index'])
            ->name('index')
            ->middleware('admin.permission:jewellery_stock.view');

        Route::get('/create', [JewelleryStockController::class, 'create'])
            ->name('create')
            ->middleware('admin.permission:jewellery_stock.create');

        Route::post('/', [JewelleryStockController::class, 'store'])
            ->name('store')
            ->middleware('admin.permission:jewellery_stock.create');

        Route::get('/check-sku', [JewelleryStockController::class, 'checkSku'])
            ->name('check-sku')
            ->middleware('admin.permission:jewellery_stock.view');
        ...
        Route::get('/{jewellery_stock}/edit', [JewelleryStockController::class, 'edit'])
            ->name('edit')
            ->middleware('admin.permission:jewellery_stock.edit');

        Route::put('/{jewellery_stock}', [JewelleryStockController::class, 'update'])
            ->name('update')
            ->middleware('admin.permission:jewellery_stock.edit');

        Route::delete('/{jewellery_stock}', [JewelleryStockController::class, 'destroy'])
            ->name('destroy')
    ...
    // ─────────────────────────────────────────────────────────────
    Route::prefix('packages')->name('packages.')->group(
        function () {
            ...
            Route::get('/lookup-stock', [PackageController::class, 'lookupStock'])
                ->name('lookup-stock')
                ->middleware('admin.permission:packages.create');
            ...
            Route::post('/{package}/return', [PackageController::class, 'returnPackage'])
                ->name('return')
    ...
    // ─────────────────────────────────────────────────────────────
    Route::prefix('shopify')->name('shopify.')->group(
        function () {
            Route::get('settings', [ShopifyController::class, 'settings'])
                ->name('settings')
                ->middleware('admin.permission:shopify.settings');
            Route::post('settings', [ShopifyController::class, 'saveSettings'])
                ->name('settings.save')
                ->middleware('admin.permission:shopify.settings');
            Route::post('test-connection', [ShopifyController::class, 'testConnection'])
            ...
            Route::post('products/import', [ShopifyController::class, 'importProducts'])
                ->name('products.import')
                ->middleware('admin.permission:shopify.products.import');
            Route::get('products/{id}', [ShopifyController::class, 'showProduct'])
                ->name('products.show')
                ->middleware('admin.permission:shopify.products.view');
            Route::post('products/{id}/export', [ShopifyController::class, 'exportProduct'])
                ->name('products.export')
                ->middleware('admin.permission:shopify.products.export');
            Route::post('products/{id}/sync', [ShopifyController::class, 'syncProduct'])
                ->name('products.sync')
                ->middleware('admin.permission:shopify.products.export');
            ...
            Route::post('collections/import', [ShopifyController::class, 'importCollections'])
                ->name('collections.import')
                ->middleware('admin.permission:shopify.collections');

            Route::get('logs', [ShopifyController::class, 'syncLogs'])
                ->name('logs')
