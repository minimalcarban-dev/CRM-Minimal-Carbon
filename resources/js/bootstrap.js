...
// Setup CSRF token immediately if available, or wait for DOMContentLoaded to be safe
const setupCsrf = () => {
...
const fallbackKey =
    typeof window !== "undefined" ? window.chatPusherKey : undefined;
const fallbackCluster =
    typeof window !== "undefined" ? window.chatPusherCluster : undefined;
...
const ensurePermissionModal = (
    ...
    document
        .getElementById("perm-revoked-btn")
        .addEventListener("click", () => safeRedirect());
...
try {
    ...
    if (connector && connector.pusher) {
        connector.pusher.connection.bind("error", (evt) => {
            const status =
                evt?.error?.data?.code || evt?.error?.status || evt?.status;
