                        ...
                        <tr>
                            ...
                            <td>
                                <div class="progress-mini">
                                    <div class="progress-mini-bar" style="width: {{$job->progress_percentage}}%;"></div>
                                    <span>{{ $job->progress_percentage }}%</span>
                                </div>
                            </td>
                            <td>
                                <div class="rows-info">
                                    <span class="success-count">✅ {{ number_format($job->successful_rows) }}</span>
                                    @if($job->failed_rows > 0)
                                        <span class="failed-count">❌ {{ number_format($job->failed_rows) }}</span>
                                    @endif
                                    <span class="total-count">/ {{ number_format($job->total_rows) }}</span>
                                </div>
                            <td>
                                <div class="time-info">
                                    {{ $job->created_at->format('M d, Y') }}<br>
                                    <small>{{ $job->created_at->format('h:i:s A') }}</small>
                                </div>
                            </td>
                            <td>
                            ...
                            <td>
                                <div class="action-btns">
                                    <a href="{{ route('diamond.job.status', $job->id) }}" class="btn-action view"
                                        title="View Details">
                                        <i class="bi bi-eye"></i>
                                    </a>
                                    @if($job->status === 'completed' && $job->type === 'export')
                                        <a href="{{ route('diamond.job.download', $job->id) }}" class="btn-action download"
                                            title="Download">
                                            <i class="bi bi-download"></i>
                                        </a>
                                    @endif
                                    @if($job->status === 'completed' && $job->type === 'import' && $job->failed_rows > 0)
                                        <a href="{{ route('diamond.download-errors', basename($job->error_file_path)) }}"
