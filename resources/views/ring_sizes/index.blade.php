    ...
    <div class="attr-list-container">
            ...
            <div class="attr-header-content">
                ...
                @if ($currentAdmin && $currentAdmin->hasPermission('ring_sizes.create'))
                    <div class="attr-header-right">
                        <a href="{{ route('ring_sizes.create') }}" class="attr-btn-create"><i class="bi bi-plus-circle"></i> Add
                            Ring Size</a>
                    </div>
                @endif
            </div>
        ...
        <div class="attr-filter-section">
            <form method="GET" class="attr-filter-form">
                ...
                @if(request('search'))<a href="{{ route('ring_sizes.index') }}" class="attr-btn-reset"><i
        ...
        <div class="attr-table-card">
                        ...
                        @foreach ($items as $item)
                            <tr class="attr-row">
                                ...
                                <td class="attr-actions">
                                    @if ($currentAdmin && $currentAdmin->hasPermission('ring_sizes.view'))<a
                                        href="{{ route('ring_sizes.show', $item->id) }}" class="attr-action-btn view"><i
                                    class="bi bi-eye"></i></a>@endif
                                    @if ($currentAdmin && $currentAdmin->hasPermission('ring_sizes.edit'))<a
                                        href="{{ route('ring_sizes.edit', $item->id) }}" class="attr-action-btn edit"><i
                                    class="bi bi-pencil"></i></a>@endif
                                    @if ($currentAdmin && $currentAdmin->hasPermission('ring_sizes.delete'))
                                        <form action="{{ route('ring_sizes.destroy', $item->id) }}" method="POST"
            ...
            @else
                <div class="attr-empty-state">
                    ...
                    <p>Get started by adding your first ring size.</p><a href="{{ route('ring_sizes.create') }}"
