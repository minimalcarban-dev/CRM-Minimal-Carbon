    ...
    <div class="attr-list-container">
        <div class="attr-list-header">
            <div class="attr-header-content">
                ...
                @if ($currentAdmin && $currentAdmin->hasPermission('stone_types.create'))
                    <div class="attr-header-right">
                        <a href="{{ route('stone_types.create') }}" class="attr-btn-create"><i class="bi bi-plus-circle"></i>
                            Add Stone Type</a>
                    </div>
                @endif
            </div>
        ...
        <div class="attr-filter-section">
            <form method="GET" class="attr-filter-form">
                ...
                @if(request('search'))
                    <a href="{{ route('stone_types.index') }}" class="attr-btn-reset"><i
        ...
        <div class="attr-table-card">
                    ...
                    <tbody>
                        @foreach ($items as $item)
                            <tr class="attr-row">
                                ...
                                <td>
                                    <span class="attr-status {{ $item->is_active ? 'active' : 'inactive' }}">
                                ...
                                <td class="attr-actions">
                                    @if ($currentAdmin && $currentAdmin->hasPermission('stone_types.view'))
                                        <a href="{{ route('stone_types.show', $item->id) }}" class="attr-action-btn view"
                                            title="View"><i class="bi bi-eye"></i></a>
                                    @endif
                                    @if ($currentAdmin && $currentAdmin->hasPermission('stone_types.edit'))
                                        <a href="{{ route('stone_types.edit', $item->id) }}" class="attr-action-btn edit"
                                            title="Edit"><i class="bi bi-pencil"></i></a>
                                    @endif
                                    @if ($currentAdmin && $currentAdmin->hasPermission('stone_types.delete'))
                                        <form action="{{ route('stone_types.destroy', $item->id) }}" method="POST"
            ...
            @else
                <div class="attr-empty-state">
                    ...
                    <p>@if(request('search')) No results match your search. @else Get started by adding your first stone type.
                    @endif</p>
                    @if(request('search'))
                        <a href="{{ route('stone_types.index') }}" class="attr-btn-create"><i
                                class="bi bi-arrow-counterclockwise"></i> Reset Filters</a>
                    @else
                        <a href="{{ route('stone_types.create') }}" class="attr-btn-create"><i class="bi bi-plus-circle"></i> Add
                            Stone Type</a>
                    @endif
                </div>
            @endif
