...
@section('title', 'Meta Integration Settings')
            ...
            <div class="header-content">
                <div class="header-left">
                    <div class="breadcrumb-nav">
                        ...
                        <a href="{{ route('leads.index') }}" class="breadcrumb-link">Leads</a>
                        <i class="bi bi-chevron-right breadcrumb-separator"></i>
        ...
        <div class="settings-card">
            ...
            <div class="webhook-info">
                ...
                <div>
                    ...
                    <div class="webhook-value">
                        <span
                            id="verifyToken">{{ $verifyToken ?: 'Not configured - add META_WEBHOOK_VERIFY_TOKEN to .env' }}</span>
                        @if($verifyToken)
                            ...
                            <tr>
                                ...
                                <td>
                                    <span class="status-badge {{ $account->is_active ? 'active' : 'inactive' }}">
                                        <i class="bi bi-{{ $account->is_active ? 'check' : 'pause' }}"></i>
                                        {{ $account->is_active ? 'Active' : 'Paused' }}
                                    </span>
                                </td>
                                <td>
                                    <div style="display: flex; gap: 0.5rem;">
                                        <form action="{{ route('settings.meta.refresh', $account) }}" method="POST"
                                        ...
                                        <button type="button" class="btn-secondary-custom" style="padding: 0.4rem 0.6rem;"
                                            onclick="toggleAccount({{ $account->id }})" title="Toggle Status">
                                            <i class="bi bi-{{ $account->is_active ? 'pause' : 'play' }}"></i>
                                        </button>
                                        <form action="{{ route('settings.meta.destroy', $account) }}" method="POST"
    ...
    <div class="modal fade" id="addAccountModal" tabindex="-1">
        <div class="modal-dialog modal-lg">
            <div class="modal-content" style="border-radius: 16px; border: none;">
                ...
                <form action="{{ route('settings.meta.store') }}" method="POST">
                    @csrf
...
@push('scripts')
    <script>
        ...
        async function testWebhook() {
            try {
                const response = await fetch('{{ route("settings.meta.test-webhook") }}', {
                    method: 'POST',
                    headers: {
