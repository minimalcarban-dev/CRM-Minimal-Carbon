    ...
    <div class="attr-list-container">
        <div class="attr-list-header">
            <div class="attr-header-content">
                ...
                @if ($currentAdmin && $currentAdmin->hasPermission('diamond_cuts.create'))
                    <div class="attr-header-right">
                        <a href="{{ route('diamond_cuts.create') }}" class="attr-btn-create"><i class="bi bi-plus-circle"></i>
                            Add Diamond Cut</a>
                    </div>
                @endif
            </div>
        ...
        <div class="attr-filter-section">
            <form method="GET" class="attr-filter-form">
                ...
                @if(request('search'))<a href="{{ route('diamond_cuts.index') }}" class="attr-btn-reset"><i
        ...
        <div class="attr-table-card">
                        ...
                        @foreach ($items as $item)
                            <tr class="attr-row">
                                ...
                                <td class="attr-actions">
                                    @if ($currentAdmin && $currentAdmin->hasPermission('diamond_cuts.view'))<a
                                        href="{{ route('diamond_cuts.show', $item->id) }}" class="attr-action-btn view"><i
                                    class="bi bi-eye"></i></a>@endif
                                    @if ($currentAdmin && $currentAdmin->hasPermission('diamond_cuts.edit'))<a
                                        href="{{ route('diamond_cuts.edit', $item->id) }}" class="attr-action-btn edit"><i
                                    class="bi bi-pencil"></i></a>@endif
                                    @if ($currentAdmin && $currentAdmin->hasPermission('diamond_cuts.delete'))
                                        <form action="{{ route('diamond_cuts.destroy', $item->id) }}" method="POST"
            ...
            @else
                <div class="attr-empty-state">
                    ...
                    <p>Get started by adding your first diamond cut.</p><a href="{{ route('diamond_cuts.create') }}"
