...
<div class="form-section-card">
    <div class="section-header">
        <div class="section-info">
            <div class="section-icon" style="background:linear-gradient(135deg,#10b981,#059669);"><i class="bi bi-calculator"></i></div>
            <div>
    ...
    <div class="section-body">
        ...
        <div style="background:var(--light-gray,#f1f5f9);border:1px solid var(--border,#e2e8f0);border-radius:10px;padding:1rem;margin-bottom:1rem;">
            <div style="display: grid; grid-template-columns: repeat(5, minmax(120px, 1fr)); gap: 1rem;">
                <div class="form-group">
                    <label class="form-label">Labor / g ($)</label>
                    <input type="number" class="form-control pricing-assumption" data-assumption="labor_rate_usd_per_gram"
                        step="0.01" value="{{ $laborDefault }}" {{ $canEditLabor ? '' : 'readonly' }}>
                </div>
                <div class="form-group">
        ...
        <div style="overflow-x: auto;">
            <table class="table table-sm align-middle" style="min-width: 1320px;">
                ...
                <tbody>
                    ...
                    $hiddenProfitPercent = $canViewProfit || $canEditProfit
                    ? old($oldBase . 'profit_percent', $row['profit_percent'] ?? $profitDefault)
                    : 0;
                    @endphp
                    <tr class="pricing-row" data-key="{{ $key }}" data-material="{{ $row['material_code'] }}"
                        ...
                        <td>
                            <input type="number" name="pricing_variants[{{ $key }}][color_weights][yellow]"
                                class="form-control pricing-input" data-field="color_weights.yellow" step="0.001"
                                value="{{ old($oldBase . 'color_weights.yellow', $row['color_weights']['yellow'] ?? 0) }}">
                        </td>
                        <td>
                            <input type="number" name="pricing_variants[{{ $key }}][color_weights][white]"
                                class="form-control pricing-input" data-field="color_weights.white" step="0.001"
                                value="{{ old($oldBase . 'color_weights.white', $row['color_weights']['white'] ?? 0) }}">
                        </td>
                        <td>
                            <input type="number" name="pricing_variants[{{ $key }}][color_weights][rose]"
                                class="form-control pricing-input" data-field="color_weights.rose" step="0.001"
                        ...
                        @endif
                        <input type="hidden" name="pricing_variants[{{ $key }}][labor_rate_usd_per_gram]"
                        ...
                        <input type="hidden" name="pricing_variants[{{ $key }}][profit_percent]"
                            data-hidden-assumption="profit_percent"
                            value="{{ $hiddenProfitPercent }}">
    ...
    function calculateJewelleryPricing() {
        ...
        document.querySelectorAll('.pricing-row').forEach(row => {
            ...
            const rate = isSilver ?
                ...
                (isPlatinum ?
                    pricingNumber(document.getElementById('platinum_950_rate_usd_per_gram')?.value || window.jewelleryPricingRates.platinum_950_usd_per_gram) :
    ...
    async function fetchJewelleryPricingRates() {
        ...
        try {
            ...
            if (payload.success) {
                ...
                status.innerText =
                    `Gold 24K adj: INR ${pricingNumber(payload.rates.gold_adjusted_inr_per_gram).toFixed(2)}/g (${money(payload.rates.gold_adjusted_usd_per_gram)}/g) | Silver base: INR ${pricingNumber(payload.rates.silver_inr_per_gram).toFixed(2)}/g (${money(payload.rates.silver_base_usd_per_gram)}/g) | Platinum 950: ${money(payload.rates.platinum_950_usd_per_gram)}/g | Source: ${payload.rates.source || 'live'}`;
            } else {
