                            ...
                            <select name="metal_type_id"
                                ...
                                @foreach ($metalTypes as $metal)
                                    <option value="{{ $metal->id }}"
                                        {{ old('metal_type_id') == $metal->id ? 'selected' : '' }}>{{ $metal->name }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                            ...
                            <select name="ring_size_id" class="form-control themed-select">
                                ...
                                @foreach ($ringSizes as $size)
                                    <option value="{{ $size->id }}"
                                        {{ old('ring_size_id') == $size->id ? 'selected' : '' }}>{{ $size->name }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:2rem;">
                                ...
                                <div style="display:flex;gap:.5rem;margin-bottom:.5rem;">
                                    <select name="certificate_type" class="form-control themed-select"
                                        ...
                                        @foreach (['GIA', 'IGI', 'HRD', 'Self'] as $ct)
                                            <option value="{{ $ct }}"
                                                {{ old('certificate_type') == $ct ? 'selected' : '' }}>{{ $ct }}
                                            </option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="form-group">
                            ...
                            <div id="image_upload_section"
                                ...
                                aria-label="Click or press Enter to upload images or videos"
                                onclick="document.getElementById('images').click()"
                                onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();document.getElementById('images').click();}"
                                onfocus="this.style.borderColor='var(--primary)';this.style.background='rgba(99,102,241,.03)'"
                                ...
                                <p style="margin:.75rem 0 0;color:var(--gray);font-weight:500;">Click to upload images or
                                    videos</p>
                                <small style="color:var(--muted);">Max 20MB · JPEG, PNG, WEBP, AVIF, HEIC, MP4, MOV</small>
                            </div>
                            <div id="image_preview_container"
                                style="display:grid;grid-template-columns:repeat(auto-fill,minmax(90px,1fr));gap:8px;margin-top:1rem;">
                            </div>
                        </div>
                    </div>
                </div>
...
@push('scripts')
    <script>
        ...
        function updateCategoryFields() {
            ...
            [ringSizeSection, thicknessSection, lengthSection, widthSection, diameterSection, baleSection, closureSection]
            .forEach(el => {
