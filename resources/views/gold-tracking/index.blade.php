...
@section('content')
    <div class="diamond-management-container tracker-page">
        ...
        @if ($factories->count() > 0)
            <div class="gold-factory-cards"
                style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 1rem; margin-bottom: 2rem;">
                @foreach ($factories as $factory)
                    <div class="gold-factory-card"
                        style="background: white; border-radius: 12px; padding: 1rem; text-align: center; border: 2px solid #e2e8f0; transition: all 0.3s;">
                        <div style="font-weight: 600; color: #1e293b; margin-bottom: 0.5rem;">{{ $factory->name }}</div>
                        <div
                            style="font-size: 1.5rem; font-weight: 700; color: {{ $factory->gold_stock > 0 ? '#f59e0b' : '#94a3b8' }};">
            ...
            <form method="GET" action="{{ route('gold-tracking.index') }}" class="tracker-filter-form"
                ...
                <div class="tracker-filter-field date-range-field">
                    ...
                    <div class="date-range-wrapper">
                        <input type="text" id="goldDateRange" class="date-range-input" placeholder="Select Date Range"
                            readonly>
                ...
                <div class="tracker-filter-field">
                    ...
                    <select name="type" class="tracker-filter-select">
                        ...
                        <option value="distribute" {{ request('type') == 'distribute' ? 'selected' : '' }}>Distribute
                        </option>
                ...
                <table class="tracker-table">
                    ...
                    <tbody>
                        ...
                        @forelse($transactions as $txn)
                            ...
                            @if ($showDateSeparator)
                                ...
                                <tr class="tracker-date-separator">
                                    <td colspan="6" style="padding: 8px 0; background: #f8fafc;">
                                        <div
                                            style="height: 1px; background: linear-gradient(to right, transparent, #e2e8f0, transparent);">
                                        </div>
                                    </td>
                                </tr>
                            @endif
                            ...
                            <tr
                                class="{{ $txn['type'] === 'purchase' ? 'tracker-income-row' : ($txn['type'] === 'return' ? '' : 'tracker-expense-row') }} {{ $isSubEntry ? 'tracker-sub-entry' : '' }}">
                                <td>
                                    @if ($isSubEntry)
                                        <span
                                            style="color: #94a3b8; padding-left: 20px; display: flex; align-items: center; gap: 6px;">
                                        ...
                                        @if ($txn['type'] === 'purchase')
                                            @if ($txn['status'] === 'pending')
                                                <button type="button" class="tracker-action-btn"
                                                    ...
                                                    onclick="openCompleteModal({{ $txn['id'] }})">
                                                    <i class="bi bi-check2-circle"></i>
                                                </button>
                                            @endif
                                            <a href="{{ route('gold-tracking.purchases.show', $txn['id']) }}"
                                                class="tracker-action-btn tracker-action-view" title="View">
                                                <i class="bi bi-eye"></i>
                                            </a>
                                            <a href="{{ route('gold-tracking.purchases.edit', $txn['id']) }}"
