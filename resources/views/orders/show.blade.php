...
<style>
    ...
    .status-custom_diamond       { background:#fef3c7; color:#92400e; }
    .status-custom_jewellery     { background:#e0e7ff; color:#3730a3; }
    .status-processed            { background:#ddd6fe; color:#5b21b6; }
    .status-completed,.status-diamond_completed { background:#d1fae5; color:#065f46; }
    .status-diamond_purchased    { background:#fce7f3; color:#9f1239; }
    .status-factory_making       { background:#fed7aa; color:#9a3412; }
    .status-priority             { background:#fee2e2; color:#991b1b; }
    .status-non_priority         { background:#f1f5f9; color:#475569; }
    .status-r_order_in_process   { background:#dbeafe; color:#1e40af; }
    .status-r_order_shipped      { background:#d1fae5; color:#065f46; }
    .status-d_diamond_in_discuss { background:#dbeafe; color:#1e40af; }
    .status-d_diamond_in_making  { background:#fef3c7; color:#92400e; }
    .status-d_diamond_completed  { background:#d1fae5; color:#065f46; }
    .status-d_diamond_in_certificate { background:#f3e8ff; color:#7c3aed; }
    .status-d_order_shipped      { background:#1e293b; color:#fff; }
    .status-j_diamond_in_progress{ background:#dbeafe; color:#1e40af; }
    .status-j_diamond_completed  { background:#d1fae5; color:#065f46; }
    .status-j_diamond_in_discuss { background:#cffafe; color:#0e7490; }
    .status-j_cad_in_progress    { background:#fef3c7; color:#92400e; }
    .status-j_cad_done           { background:#f3e8ff; color:#7c3aed; }
    .status-j_order_completed    { background:#d1fae5; color:#065f46; }
    .status-j_order_in_qc        { background:#fef3c7; color:#92400e; }
    .status-j_qc_done            { background:#d1fae5; color:#065f46; }
    .status-j_order_shipped      { background:#1e293b; color:#fff; }
    .status-j_order_hold         { background:#fee2e2; color:#991b1b; }
    .status-delivered            { background:#d1fae5; color:#065f46; }
    .status-in_transit           { background:#dbeafe; color:#1e40af; }
    .status-out_for_delivery     { background:#fae8ff; color:#86198f; }
    .status-failed_attempt       { background:#fee2e2; color:#be123c; }
    ...
    .od-card:hover { box-shadow: var(--shadow); }
    ...
    /* ── Info Rows ───────────────────────────── */
    .od-info-table { display: flex; flex-direction: column; }
    .od-info-row {
        display: grid;
    ...
    .od-info-row:first-child { padding-top: 0; }
    ...
    .od-info-val.address { white-space: pre-line; }

    /* ── Detail Group ────────────────────────── */
    .od-detail-group { margin-bottom: 1rem; }
    .od-detail-group:last-child { margin-bottom: 0; }
    .od-detail-label {
        display: flex;
        align-items: center;
        gap: .35rem;
    ...
    .od-tl { position: relative; padding-left: 1.375rem; }
    .od-tl::before {
        content: '';
        position: absolute;
    ...
    .od-img-item img { width: 100%; height: 100%; object-fit: cover; display: block; }
    .od-img-overlay {
        position: absolute;
        inset: 0;
    ...
    .od-img-item:hover .od-img-overlay { opacity: 1; }
    ...
    .od-pdf-item:last-child { margin-bottom: 0; }
    .od-pdf-item:hover { border-color: #c7d2fe; background: var(--primary-soft); }
    .od-pdf-icon {
    ...
    .od-pdf-info { flex: 1; min-width: 0; }
    .od-pdf-name {
        font-size: .9375rem;
        font-weight: 600;
        color: var(--dark);
    ...
    .od-pdf-size { font-size: .8125rem; color: var(--muted); }
    .od-pdf-actions { display: flex; gap: .375rem; flex-shrink: 0; }
    .od-pdf-btn {
    ...
    .od-edit-tl { display: flex; flex-direction: column; gap: 1rem; }
    .od-edit-item {
        border: 1.5px solid var(--border);
        border-radius: var(--radius-sm);
        overflow: hidden;
    ...
    .od-edit-item:hover { border-color: #c7d2fe; }
    .od-edit-header {
        display: flex;
        align-items: center;
        gap: .75rem;
    ...
    .changes-toggle:hover { background: var(--primary-soft); }
    .toggle-icon { margin-left: auto; transition: transform .2s; }
    .timeline-changes.expanded .toggle-icon { transform: rotate(180deg); }
    .changes-detail { display: none; }
    .timeline-changes.expanded .changes-detail { display: block; }
    .changes-table {
        width: 100%;
        font-size: .875rem;
    ...
    .field-name { font-weight: 600; color: var(--dark); }
    .val-badge {
    ...
    .val-new { background: var(--success-soft); color: #065f46; }
    ...
    .od-modal-close:hover { background: var(--danger-soft); border-color: var(--danger); color: var(--danger); }
    .od-modal-body {
        flex: 1;
        overflow: hidden;
        min-height: 0;
    ...
    .od-modal-body iframe, .od-modal-body img {
    ...
    .spin { animation: od-spin 1s linear infinite; display: inline-block; }
    ...
    .od-stepper-heading i { color: var(--primary); }
    .od-stepper-sync {
        font-size: .75rem;
        color: var(--muted);
        display: flex;
    ...
    .od-step.active .od-step-date { color: var(--primary); font-weight: 600; }
    ...
    .od-stepper-all-toggle i { transition: transform .25s; }
    .od-stepper-all-toggle.open i { transform: rotate(180deg); }
    ...
    .od-stepper-all-list.visible { display: block; }
    .od-sal-item {
        display: grid;
    ...
    .od-sal-item:last-child { border-bottom: none; }
    .od-sal-date {
        font-size: .72rem;
        font-family: var(--font-mono);
        color: var(--muted);
    ...
    @media (max-width: 640px) {
        ...
        .od-order-id { font-size: 1.375rem; }
        .od-meta-strip { gap: .5rem; }
        .od-meta-card { min-width: 100px; }
    }
</style>
    ...
    <div class="od-grid">
                ...
                <div class="od-card-body">
                    <div class="od-info-table">
                        <div class="od-info-row">
                            ...
                            <span class="od-info-val">{{ $order->display_client_name ?? ($order->client_details ?? 'N/A') }}</span>
                        </div>
                        <div class="od-info-row">
                        ...
                        <div class="od-info-row">
                            <span class="od-info-key">{{ \App\Models\Order::TAX_ID_TYPES[$order->client_tax_id_type] ?? 'Tax ID' }}</span>
                    ...
                    <div class="od-melee-box">
                        ...
                        <div class="od-melee-grid">
                            <div class="od-melee-item" style="grid-column: span 2;">
                                ...
                                <span>{{ $order->meleeDiamond->category->name ?? 'Melee' }} — {{ str_replace('-', ' ', $order->meleeDiamond->size_label ?? 'N/A') }}</span>
                            </div>
                            ...
                            <div class="od-melee-item">
                                ...
                                <span>$ {{ number_format($order->melee_total_value ?? ($order->melee_carat * $order->melee_price_per_ct), 2) }}</span>
                            </div>
                            @endif
                        </div>
                    </div>
        ...
        <div class="od-col">
            ...
            <div class="od-card">
                <div class="od-card-head">
                    <h3 class="od-card-title">
                        ...
                        <span style="font-size:.72rem; background:var(--primary-soft); color:var(--primary); padding:.1rem .45rem; border-radius:100px; font-weight:700; margin-left:.25rem;">{{ count($images) }}</span>
                    </h3>
                </div>
                <div class="od-card-body">
            ...
            @php
                ...
                if (is_string($pdfs)) { $pdfs = json_decode($pdfs, true); }
            ...
            <div class="od-card">
                <div class="od-card-head">
                    <h3 class="od-card-title">
                        ...
                        <span style="font-size:.72rem; background:#fee2e2; color:#dc2626; padding:.1rem .45rem; border-radius:100px; font-weight:700; margin-left:.25rem;">{{ count($pdfs) }}</span>
                    </h3>
                </div>
                <div class="od-card-body">
                    @foreach($pdfs as $pdf)
                ...
                <div class="od-card-body">
                    <div class="od-ship-grid">
                        ...
                        <div class="od-spec">
                            ...
                            <span class="od-spec-val" style="font-family: var(--font-mono); font-size:.8rem;">{{ $order->tracking_number }}</span>
                        </div>
                        @endif
                        @if($order->dispatch_date)
                        <div class="od-spec">
                            <span class="od-spec-label">Dispatch Date</span>
                            <span class="od-spec-val">{{ \Carbon\Carbon::parse($order->dispatch_date)->format('d M Y') }}</span>
                        </div>
                        @endif
                        @if($order->tracking_status)
                        <div class="od-spec">
                            <span class="od-spec-label">Live Status</span>
                            <span class="s-badge status-{{ strtolower(str_replace(' ', '_', $order->tracking_status)) }}">
                    ...
                    @php
                        ...
                        $milestones = [
                            ...
                            'in_transit'   => ['label' => 'In Transit',   'icon' => 'bi-airplane',        'keywords' => ['transit','departure','arrival','sorting','airport','processing','processed','facility','hub']],
                            'customs'      => ['label' => 'Customs',      'icon' => 'bi-shield-check',    'keywords' => ['custom','clearance','cleared','held']],
                            'out_delivery' => ['label' => 'Out for Del.', 'icon' => 'bi-truck',           'keywords' => ['out_for','out for','delivery','delivering']],
                            'delivered'    => ['label' => 'Delivered',    'icon' => 'bi-check-circle',    'keywords' => ['delivered','received','complete']],
                        ];
                        ...
                        // Active step = last reached
                        $milestoneKeys = array_keys($milestones);
                        $activeStep = 'dispatched';
                        foreach ($milestoneKeys as $k) {
                            if (isset($reached[$k])) $activeStep = $k;
                        }
                    @endphp
                    ...
                    <div class="od-stepper-wrap">
                        ...
                        <div class="od-stepper">
                            @foreach($milestones as $key => $m)
                            @php
                                $isDone   = isset($reached[$key]) && $key !== $activeStep;
                                ...
                                <div class="od-step-body">
                                    ...
                                    @if($stepEvent)
                                        <span class="od-step-date">
                                            {{ \Carbon\Carbon::parse($stepEvent['date'] ?? '')->format('d M') ?? '' }}
                                        </span>
                                    @endif
                                </div>
                            </div>
                        ...
                        <div class="od-stepper-all-list">
                            ...
                            <div class="od-sal-item">
                                ...
                                <div>
                                    ...
                                    <div class="od-sal-loc"><i class="bi bi-geo-alt-fill"></i> {{ $ev['location'] }}</div>
                                    @endif
            ...
            <div class="od-card">
                <div class="od-card-head">
                    <h3 class="od-card-title">
                        ...
                        <span style="font-size:.72rem; background:var(--primary-soft); color:var(--primary); padding:.1rem .45rem; border-radius:100px; font-weight:700; margin-left:.25rem;">{{ $editHistory->count() }}</span>
                    </h3>
                </div>
                                    ...
                                    <table class="changes-table">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td><span class="val-badge val-old">{{ Str::limit($log->old_values[$field] ?? '—', 80) }}</span></td>
                                                <td><span class="val-badge val-new">{{ Str::limit($newVal ?? '—', 80) }}</span></td>
                                            </tr>
                                            @endforeach
                                        </tbody>
                                    </table>
...
<div id="pdfModal" class="od-modal no-print" onclick="closePDFModal()">
    <div class="od-modal-box" onclick="event.stopPropagation()">
        ...
        <div class="od-modal-body">
            <iframe id="pdfViewer" src="" frameborder="0" style="width:100%; height:100%; min-height:0; flex:1;"></iframe>
        </div>
    </div>
</div>
