    ...
    <div class="attr-list-container">
            ...
            <div class="attr-header-content">
                ...
                @if ($currentAdmin && $currentAdmin->hasPermission('metal_types.create'))
                    <div class="attr-header-right">
                        <a href="{{ route('metal_types.create') }}" class="attr-btn-create"><i class="bi bi-plus-circle"></i>
                            Add Metal Type</a>
                    </div>
                @endif
            </div>
        ...
        <div class="attr-filter-section">
            <form method="GET" class="attr-filter-form">
                ...
                @if(request('search'))<a href="{{ route('metal_types.index') }}" class="attr-btn-reset"><i
        ...
        <div class="attr-table-card">
                        ...
                        @foreach ($items as $item)
                            <tr class="attr-row">
                                ...
                                <td class="attr-actions">
                                    @if ($currentAdmin && $currentAdmin->hasPermission('metal_types.view'))<a
                                        href="{{ route('metal_types.show', $item->id) }}" class="attr-action-btn view"><i
                                    class="bi bi-eye"></i></a>@endif
                                    @if ($currentAdmin && $currentAdmin->hasPermission('metal_types.edit'))<a
                                        href="{{ route('metal_types.edit', $item->id) }}" class="attr-action-btn edit"><i
                                    class="bi bi-pencil"></i></a>@endif
                                    @if ($currentAdmin && $currentAdmin->hasPermission('metal_types.delete'))
                                        <form action="{{ route('metal_types.destroy', $item->id) }}" method="POST"
            ...
            @else
                <div class="attr-empty-state">
                    ...
                    <p>Get started by adding your first metal type.</p><a href="{{ route('metal_types.create') }}"
